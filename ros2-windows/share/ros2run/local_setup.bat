...
set "AMENT_ENVIRONMENT_HOOKS[ros2run]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[ros2run] "%AMENT_CURRENT_PREFIX%\share\ros2run\environment\ament_prefix_path.bat;%AMENT_CURRENT_PREFIX%\share\ros2run\environment\path.bat;%AMENT_CURRENT_PREFIX%\share\ros2run\environment\pythonpath.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[ros2run]:;=";"%") do (
    call:call_file %%f
  )
