...
set "AMENT_ENVIRONMENT_HOOKS[launch_testing]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[launch_testing] "%AMENT_CURRENT_PREFIX%\share\launch_testing\environment\ament_prefix_path.bat;%AMENT_CURRENT_PREFIX%\share\launch_testing\environment\path.bat;%AMENT_CURRENT_PREFIX%\share\launch_testing\environment\pythonpath.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[launch_testing]:;=";"%") do (
    call:call_file %%f
  )
