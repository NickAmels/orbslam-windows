...
set "AMENT_ENVIRONMENT_HOOKS[topic_monitor]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[topic_monitor] "%AMENT_CURRENT_PREFIX%\share\topic_monitor\environment\ament_prefix_path.bat;%AMENT_CURRENT_PREFIX%\share\topic_monitor\environment\path.bat;%AMENT_CURRENT_PREFIX%\share\topic_monitor\environment\pythonpath.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[topic_monitor]:;=";"%") do (
    call:call_file %%f
  )
