...
set "AMENT_ENVIRONMENT_HOOKS[ros2node]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[ros2node] "%AMENT_CURRENT_PREFIX%\share\ros2node\environment\ament_prefix_path.bat;%AMENT_CURRENT_PREFIX%\share\ros2node\environment\path.bat;%AMENT_CURRENT_PREFIX%\share\ros2node\environment\pythonpath.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[ros2node]:;=";"%") do (
    call:call_file %%f
  )
