...
set "AMENT_ENVIRONMENT_HOOKS[fastcdr]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[fastcdr] "%AMENT_CURRENT_PREFIX%\share\fastcdr\environment\ament_prefix_path.bat;%AMENT_CURRENT_PREFIX%\share\fastcdr\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[fastcdr]:;=";"%") do (
    call:call_file %%f
  )
