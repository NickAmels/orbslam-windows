...
set "AMENT_ENVIRONMENT_HOOKS[urdfdom]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[urdfdom] "%AMENT_CURRENT_PREFIX%\share\urdfdom\environment\ament_prefix_path.bat;%AMENT_CURRENT_PREFIX%\share\urdfdom\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[urdfdom]:;=";"%") do (
    call:call_file %%f
  )
