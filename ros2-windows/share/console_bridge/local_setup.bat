...
set "AMENT_ENVIRONMENT_HOOKS[console_bridge]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[console_bridge] "%AMENT_CURRENT_PREFIX%\share\console_bridge\environment\ament_prefix_path.bat;%AMENT_CURRENT_PREFIX%\share\console_bridge\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[console_bridge]:;=";"%") do (
    call:call_file %%f
  )
