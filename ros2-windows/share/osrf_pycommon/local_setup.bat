...
set "AMENT_ENVIRONMENT_HOOKS[osrf_pycommon]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[osrf_pycommon] "%AMENT_CURRENT_PREFIX%\share\osrf_pycommon\environment\ament_prefix_path.bat;%AMENT_CURRENT_PREFIX%\share\osrf_pycommon\environment\path.bat;%AMENT_CURRENT_PREFIX%\share\osrf_pycommon\environment\pythonpath.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[osrf_pycommon]:;=";"%") do (
    call:call_file %%f
  )
