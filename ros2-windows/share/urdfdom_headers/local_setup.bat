...
set "AMENT_ENVIRONMENT_HOOKS[urdfdom_headers]="

:: these are the environment hooks of this package
call:ament_append_value AMENT_ENVIRONMENT_HOOKS[urdfdom_headers] "%AMENT_CURRENT_PREFIX%\share\urdfdom_headers\environment\ament_prefix_path.bat;%AMENT_CURRENT_PREFIX%\share\urdfdom_headers\environment\path.bat"
...
if not defined AMENT_RETURN_ENVIRONMENT_HOOKS (
  ...
  for %%f in ("%AMENT_ENVIRONMENT_HOOKS[urdfdom_headers]:;=";"%") do (
    call:call_file %%f
  )
